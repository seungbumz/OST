...
rf2xx_wr_channel_clear(void)
{
    ...
    switch (rf2xx_state)
    {
        ...
        case RF_LISTEN:
            ...
            //reg = rf2xx_reg_read(RF2XX_DEVICE, RF2XX_REG__PHY_CC_CCA);
            reg &= RF2XX_PHY_CC_CCA_MASK__CCA_MODE;
            //printf("CCA_MODE when CCA %02x %u, %02x\n", reg,reg, RF2XX_PHY_CC_CCA_DEFAULT__CCA_MODE);
            // wait cca to be done
            do
            {
                platform_enter_critical();
...
rf2xx_wr_receiving_packet(void)
{
  if(!RF2XX_WITH_TSCH) {
    ...
    //JSB : Datasheet p.39
    //JSB: Preamble +SFD+ PHR +PSDU
    //JSB : Valid PHR detection (RX_START), or Start of PSDU  ||  SHR (or SFD) detection
  }
}
...
/*
    ...
    reg &= RF2XX_TRX_CTRL_1_MASK__TX_AUTO_CRC_ON;
    printf("TX_AUTO_CRC ON %02x %u\n", reg,reg);

    reg = rf2xx_reg_read(RF2XX_DEVICE, RF2XX_REG__PHY_CC_CCA);
    reg &= RF2XX_PHY_CC_CCA_MASK__CCA_MODE;
    printf("CCA_MODE %02x %u\n", reg,reg);

    reg = rf2xx_reg_read(RF2XX_DEVICE, RF2XX_REG__CCA_THRES);
    reg &= 0x0f;
    printf("CCA_ED_THRES %02x %u\n",reg,reg);
*/
    /*********************END******************/
...
#if IOT_LAB_M3
  case RADIO_PARAM_LAST_RSSI: //JSB add
